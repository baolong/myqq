...
int main()
{
    ...
    struct User_List *Cur[100];     //存储用户当前节点
    ...
    while(sumofcli < 100)
    {
        ...
        if ((fd_[sumofcli] = Accept(fd,&cli_addr[sumofcli])) > 0)
        {
            ...
            argv[sumofcli]->cur = Cur[sumofcli];
...
//    Send(fd,FriendList);
    ...
    OnLine(cur,name,3);     //设置为离线
    SetSocket(cur,name,0);   //清楚套接字描述符
    close(fd);
}
...
//      goto LOGIN_AGAIN;
        else
        {
            ...
            while(0 != strcmp(argv->cur->user.name,argv->name) && NULL != argv->cur->next)
                argv->cur = argv->cur->next;
            Running(argv->user,argv->fd_,argv->cur,argv->name,argv->passwd);
        }
    }
}
